...
//                                                                            //
//   Created: 2026/01/20 11:49:20 by jeportie                                 //
//   Updated: 2026/01/20 11:49:31 by jeportie                                 //
//                                                                            //
// ************************************************************************** //
...
export type FilterRoutes<Schema, M extends HttpMethods> = {
  [E in EndpointKey<Schema>]: Schema[E] extends Record<MethodsMap[M], unknown>
    ? E
    : never;
}[EndpointKey<Schema>];
...
type ResponsesOf<O, S extends number> = O extends { responses?: infer R }
  ? S extends keyof R
    ? R[S]
    : never
  : never;
...
/**
 * ApiRegistry interface that maps base URLs to their OpenAPI schema types.
 * Users can augment this interface in their project using module augmentation.
 *
 * @example
 * ```typescript
 * // In your project's src/apiRegistry.ts
 * import type { paths as MyAPI } from './schema/my-api.js';
 *
 * declare module '@jeportie/via' {
 *   interface ApiRegistry {
 *     'https://api.example.com': MyAPI;
 *   }
 * }
 * ```
 */
